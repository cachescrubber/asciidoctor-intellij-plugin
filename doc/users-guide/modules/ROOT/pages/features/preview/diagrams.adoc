...
A Kroki diagram can fail in the preview for three reasons: the block macro's target cannot be resolved or
read, one of its `+!include+` files does not exist, or the Kroki server rejects the diagram.
In all three cases the preview shows a placeholder box instead of an error paragraph or a silently
incomplete diagram (the Kroki server drops an include it cannot read and renders the rest).
