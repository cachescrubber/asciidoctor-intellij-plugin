...
- Resolve Antora `example$` resource ids as Kroki diagram block macro targets inside an Antora module, and show a placeholder with an open-in-editor link for Kroki diagrams the preview cannot render (unresolvable target, missing include file, server error) instead of an error paragraph or a silently degraded diagram (#516)
